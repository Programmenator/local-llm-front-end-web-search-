...
class SessionService:
    ...
    def load_session(self, session_id: str) -> Optional[ConversationSession]:
        ...
        This method now performs a direct filename lookup pattern instead of
        calling ``list_sessions()`` and deserializing every saved session file.
        That keeps background worker completions from degrading to an O(N)
        full-disk scan as the session count grows.
        """
