...
class ChatController:
    ...
    def update_web_search_enabled(self, enabled: bool) -> dict:
        """Persist the main-window internet-search toggle immediately.

        Why this method exists:
            The main window now includes a visible internet-search toggle so the
            user can tell, without opening Settings, whether the model is
            allowed to call web tools. This method updates only that flag and
            refreshes the view without forcing a full settings-save cycle.
        """
        self._raise_if_generation_active("change the internet-search setting")
        saved = self.config_service.update_config({"enable_web_search": bool(enabled)})
        if self.view:
            self.view.refresh_model_display(saved)
        return saved
    ...
    def unload_selected_model(self, model_name: str) -> dict:
        ...
        return {
            ...
            "message": (
                ...
                "Ollama did not report an explicit done flag, so verify with ollama ps if needed."
            ),
        }
    ...
    def get_gpu_metrics_snapshot(self) -> dict:
        ...
        Why this is exposed through the controller:
            ...
            simply forwards the normalized snapshot from the service layer.
        """
        return self.gpu_monitor_service.get_live_metrics()
    ...
    def _refresh_models_background(self) -> None:
        ...
        if self.view:
            self.view.safe_ui_call(lambda: self.view.update_model_choices(models, str(config.get("model", ""))))
    ...
    def _auto_title_if_needed(self, session: ConversationSession) -> None:
        ...
        """
        if not re.fullmatch(r"Session \d{4}-\d{2}-\d{2} \d{2}:\d{2} (?:AM|PM)", session.title):
            return
    ...
    def _create_generation_job(self, user_message_text: str) -> GenerationJob:
        ...
        return GenerationJob(
            ...
            enable_search_reranker=bool(config.get("enable_search_reranker", False)),
            search_reranker_backend=str(config.get("search_reranker_backend", "disabled")),
            search_reranker_model=str(config.get("search_reranker_model", "")),
            sentence_transformers_reranker_model=str(
                config.get("sentence_transformers_reranker_model", "BAAI/bge-reranker-base")
            ),
            sentence_transformers_reranker_device=str(
                config.get("sentence_transformers_reranker_device", "auto")
            ),
            web_max_pages=int(config.get("web_max_pages", 2)),
            web_fetch_enabled=bool(config.get("web_fetch_enabled", True)),
            web_fetch_max_response_bytes=int(config.get("web_fetch_max_response_bytes", 1500000)),
            web_fetch_max_redirects=int(config.get("web_fetch_max_redirects", 4)),
            web_fetch_allowlist=str(config.get("web_fetch_allowlist", "")),
            web_fetch_blocklist=str(config.get("web_fetch_blocklist", "")),
