...
class ConfigService:
    ...
    def _merge_with_defaults(self, raw_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        merged["reranker_model"] = str(merged.get("reranker_model", DEFAULT_CONFIG["reranker_model"])).strip()
        ...
        merged["web_max_results"] = max(
            1,
            min(200, self._safe_int(merged.get("web_max_results"), int(DEFAULT_CONFIG["web_max_results"]))),
        )
        merged["web_max_pages"] = max(
            1,
            min(50, self._safe_int(merged.get("web_max_pages"), int(DEFAULT_CONFIG["web_max_pages"]))),
        )
